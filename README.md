...
| `/bearings`        | Generate a concise four-section chat digest with live verification of every named PR; use `/bearings file` to replace today's dated report in `data/`, and add `include PRs` to discover additional open PRs |
...
- `/bearings file include PRs` combines the dated report with additional open-PR discovery.
