...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  <id>/quota.jsonl   provider quota ledger: one record at spawn, one at close; survives teardown, so what a task cost is readable afterward with `bin/fm-quota-delta.sh <id>` or `--fleet`
