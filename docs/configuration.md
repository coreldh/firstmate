...
Publication exclusively creates each single-link mode-0400 artifact, accepts an existing artifact only when its bytes and ownership shape are identical, and never overwrites or deletes an earlier generation.
Publication is not all-or-none, so a failed attempt may leave a partial generation directory or artifact.
Such residue is not authoritative because dispatch never scans generation paths or treats their existence as authority; only the `routing_decision` pointer written to successful task metadata can authorize flag-free inheritance, and that pointer must name a non-symlink regular receipt whose task-scoped generation matches its bytes.
...
The control-plane preflight for a route-changing relaunch is the exception: it publishes the receipt before any checkpoint journal or agent exit, then passes the exact published paths and launch-input bytes through the existing control handoff so the launch half cannot perform a second routing validation after effects.
That publication applies the same five-minute acceptance check without widening it; once accepted, the receipt is not re-aged during the bounded control handoff, and its artifacts remain even if a later non-routing checkpoint, stop, or launch operation fails.
Routing refusals always precede every side effect; non-routing preflight failures may occur after receipt publication and leave non-authoritative artifacts, including Kimi hook installation after ordinary publication and Muse credential failure after a published control handoff.
...
Once successful task metadata points at a generation, that generation cannot authorize another route-changing attempt; a fresh route-changing dispatch therefore requires different receipt bytes and a fresh generation.
