...
Before publishing a generation, dispatch exclusively creates or opens the task-scoped mode-0600 `routing-generations.consumed` ledger through a no-follow boundary, takes an exclusive lock, validates every existing row as one lowercase SHA-256 generation, appends and synchronizes the new generation, and verifies that the pathname still identifies the held ledger.
