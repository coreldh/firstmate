...
The wrapper uses `FM_ROOT_OVERRIDE` only for that checkout classification and always resolves `fm-cd-command-policy.mjs` beside the trusted wrapper itself.
...
| Codex | `.codex/hooks.json` PreToolUse hook that resolves the checker through [`architecture.md`](architecture.md#codex-project-hook-payload-integrity) and forwards the payload | Blocks on exit 2 and displays stderr. |
