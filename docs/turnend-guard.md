...
- Codex registers a `Stop` hook in `.codex/hooks.json`, executes the manifest-verified staged guard through the [Codex hook payload boundary](architecture.md#codex-hook-payload-boundary), and passes the original payload to the shared guard.
...
`tests/fm-codex-hook-integrity.test.sh` covers Codex payload refusal, prepared-root continuity, non-login invocation, scoped macOS denial, reversible user flags, and the staged execution race regression.
