...
- Codex registers a `Stop` hook in `.codex/hooks.json`, resolves the executable through the anchored and manifest-verified payload boundary owned by [`architecture.md`](architecture.md#codex-project-hook-payload-integrity), and passes the original payload to the shared guard.
...
`tests/fm-turnend-guard.test.sh` covers the predicate, main and secondmate primary scope, child-worktree exclusion, `FM_HOME` and `FM_STATE_OVERRIDE` precedence, the cooperative `--claude` claim wait, epoch allow, re-block budget, Pi logical-run latching, missing-`jq` behavior, all five primary registrations, Codex payload refusal, Grok native and legacy selection, typed field precedence, malformed input, and exactly-one-path safety.
`tests/fm-codex-hook-integrity.test.sh` is the offline executable reproduction of the legacy worktree-payload bypass and the anchored, manifest-verified refusal.
