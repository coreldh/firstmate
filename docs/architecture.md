...
The Codex project-hook trust boundary, including the login-shell profile vector, writable anchor, check-to-execution race, alternatives, and rollback requirements, is owned by [codex-hook-threat-model.md](codex-hook-threat-model.md).
...
After the complete stow pass, `bin/fm-captain-resume.sh` atomically regenerates the canonical home-root handoff from fresh structured state; [captain-resume.md](captain-resume.md) owns its current operator contract.
