...
Every covered dispatch crosses the unconditional task-scoped receipt boundary in `fm-spawn.sh`, independently of whether canonical dispatch configuration exists.
[`configuration.md`](configuration.md#routing_decision-receipts) owns the receipt schemas, canonical configuration binding, selected-versus-emitted command axes, raw-launch restrictions, and relaunch exemptions.
