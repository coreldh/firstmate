...
Every bearings generation live-checks each PR URL that survives into its bounded output through `gh-axi`; forge failures remain explicit unknown, non-actionable state.
The `--include-prs` opt-in only adds bounded discovery of open PRs that were not already named by local fleet records.
