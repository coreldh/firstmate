...
| `fm-quota-record.sh`     | Append one provider quota record to a task's durable ledger at spawn or at close     |
| `fm-quota-delta.sh`      | Report what a task cost in provider quota, per window, for one task or the fleet     |
