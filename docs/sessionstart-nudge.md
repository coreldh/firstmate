...
| Codex | `.codex/hooks.json` executes the manifest-verified staged wrapper through the [Codex hook payload boundary](architecture.md#codex-hook-payload-boundary). | Native stdout context injection is supported when project hooks load; current live limitations remain in the verification record. |
...
`tests/fm-turnend-guard.test.sh`, `tests/fm-codex-hook-integrity.test.sh`, `tests/fm-pi-watch-extension.test.sh`, and `tests/fm-daemon.test.sh` cover marked guard, verified Codex payload delivery, monitoring, and away-mode delivery.
