...
1. **Run the updater:**
   ...
   It prints one status line per target (`updated <old>..<new>` / `already current` / `skipped: <reason>`), any `HOOK_MANIFEST_STALE` or `HOOK_MANIFEST_ERROR` diagnostics for an updated root, and two action lines that tell you exactly what to do next:
   - `reread-firstmate: yes|no`
   - `nudge-secondmates: fm-<id>...|none`

2. **Handle every hook-manifest diagnostic.**
   A `HOOK_MANIFEST_STALE` or `HOOK_MANIFEST_ERROR` line means the named updated root cannot safely run its Codex hooks.
   Do not describe that root as successfully updated and do not send its re-read nudge.
   Report the named root and diagnostic to the captain, and route regeneration with `bin/fm-hook-manifest.sh` through the normal tracked Firstmate change and review path rather than dirtying the updated root in place.
...
4. **Nudge each updated live secondmate.**
   For every target listed on the `nudge-secondmates:` line except a root named by a hook-manifest diagnostic (do nothing when it says `none`), send a one-line re-read nudge so that secondmate picks up its new instructions too:
   ```sh
   ...
   A secondmate that was skipped, already current, has no live metadata, or was named by a hook-manifest diagnostic needs no nudge.
