...
# Both walks below strip the directory from a process comm with ${comm##*/}
# rather than basename. A login shell records argv[0] with a leading dash
# ("-zsh", "-/bin/bash") and every ancestry walk passes one, where an unguarded
# BSD basename reads that operand as an option cluster and writes "illegal
# option" to stderr on every turn end. "basename --" also fixes that; parameter
# expansion is preferred here because it cannot misparse an operand at all and
# saves a fork per hop on a path that runs at every turn end.
